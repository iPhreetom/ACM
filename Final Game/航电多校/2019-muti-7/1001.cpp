...
void output (const string& tempa, const string& tempb, bool sw) {
    int la = tempa.size()-1;
    int lb = tempb.size()-1;
    int lc = tempc.size()-1;
    int rabishi = min(a.size(),min(b.size(),c.size()));

    for (int i=0; i<min(tempa.size(),min(tempb.size(),tempc.size())); i++){
        if (la-i <= a.size() || lb-i <= b.size() || lc-i <= c.size()) {
            rabishi = i;
            break;
        }

        if (tempa[la-i]=='0' && tempb[lb-i]=='0' && tempc[lc-i]=='0') {
            continue;
        }
        else {
            rabishi = i;
    ...
    if (sw) {
        cout << lb-(int)b.size()-rabishi << ' '
        << la-(int)a.size()-rabishi << ' '
        << lc-(int)c.size()-rabishi << endl;
    }
    else {
        cout << la-a.size()-rabishi << ' '
        << lb-b.size()-rabishi << ' '
        << lc-c.size()-rabishi << endl;
    }
}

void getans(int numb, bool sw) {
    string tempb = b;
    while(tempb.size() < tempc.size() - numb) tempb.push_back('0');

    Bignum B = tempb;
    if (B < C) {
        B = C-B;
        const string& tempa = B.to_string();
        bool can = 1;
