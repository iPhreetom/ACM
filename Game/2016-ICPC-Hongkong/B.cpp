...
// }
//
// double pp( point const&a,point const&b){
...
signed main(){
	...
	while(tt--){
		 ...
		 // kozi
		 if(af*2 <= pi){
			 ans = min(ans, disptoline(up,as,ae));
		 }
		 else{
			 ans = min(l,ans);
		 }
		 ...
		 if(bt*2 <= pi){
			 ...
			 if(af*2 <= pi){
				 if(bt >= af){
					 // as,ae,be
					 ans = min(ans,disptoline(be,as,ae));
					 // cout<<"ans = "<<ans<<endl;
				 }
			 }

			 // now is  as to bs be
			 ans = min(ans,disptoline(as,bs,be));
		 }
		 else{
			 // can pass the kozi
			 // now judge b and line
			 // cout<<"up.x << ' ' << up.y = "<<up.x << ' ' << up.y<<endl;
			 // cout<<"as.x << ' ' << as.y = "<<as.x << ' ' << as.y<<endl;
			 // cout<<"be.x << ' ' << be.y = "<<be.x << ' ' << be.y<<endl;
			 // ans = min(ans,disptoline(as,bs,b/e));
			 ans = min(ans,w-be.y);
		 }
		 if(ans < 0)ans = 0;
		 cout<<ans/2<<endl;
